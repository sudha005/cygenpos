...
class Import extends MY_Controller {
    ...
    public function itemmodifiers(){
	   $this->permission_check('import_items');
		$data=$this->data;
		$data['page_title']='Import Item Modifiers';
		$this->load->view('import/import_item_modifiers', $data);
	}
	
	public function import_item_modifiers_csv() {
                extract($this->xss_html_filter(array_merge($this->data)));
                $filename = $_FILES["import_file"]["name"];
                ...
                {   
                	...
                	$config['upload_path']          = './uploads/csv/itemmodifiers';
	                $config['allowed_types']        = 'csv';
	                $this->load->library('upload', $config);
                    ...
                    $file = fopen('uploads/csv/itemmodifiers/'.$file_name,"r");
                    ...
                    while(($importdata = fgetcsv($file, NULL, ",")) !== FALSE){
                        ...
                        $q1=$this->db->query($qs4);
                        $maxid=$q1->row()->maxid;
                        $items_modifier_code='MD'.str_pad($maxid, 4, '0', STR_PAD_LEFT);
                        //end

                        $item_id=$importdata[0];
                        $modifier_id=$importdata[1];
                        //Validate This modifier already exist or not
                        $query=$this->db->query("select * from db_items_modifier where upper(item_id)=upper('$item_id') and upper(modifier_id)=upper('$modifier_id')");
                        if($query->num_rows()>0){
                            echo "Import Failed!<br>'".$item_id."'' Item Modifier already Exist.<br>Row Number:".$i++;
                            exit();
                        }
                        ...
                        //If any record failed to save flag will be set false,then all records rolled back
                        if(!$this->db->insert('db_items_modifier',$row)){
                            $flag='false';
                        }
                    ...
                    if(!$flag){
                        ...
                        $this->db->trans_commit();
                        echo "success";
                        $this->session->set_flashdata('success', 'Success!! Item Modifiers Data Imported Successfully!');
                    }
                    fclose($file);
                }
         ...
         public function itemmodifiers_group(){
	   $this->permission_check('import_items');
		$data=$this->data;
		$data['page_title']='Import Item Modifiers Group';
		$this->load->view('import/import_item_modifiers_group', $data);
	}
	
	
		public function import_item_modifiersgroup_csv() {
                extract($this->xss_html_filter(array_merge($this->data)));
                $filename = $_FILES["import_file"]["name"];
                ...
                {   
                	...
                	$config['upload_path']          = './uploads/csv/itemmodifiersgroup';
	                $config['allowed_types']        = 'csv';
	                $this->load->library('upload', $config);
                    ...
                    $file = fopen('uploads/csv/itemmodifiersgroup/'.$file_name,"r");
                    ...
                    while(($importdata = fgetcsv($file, NULL, ",")) !== FALSE){
                        ...
                        $row = array(
                            'item_id'     =>  $item_id,
                            'modifiergroup_id'     =>  $modifier_id,
                            'status'                    => 1,
                        );
                        
                        //If any record failed to save flag will be set false,then all records rolled back
                        if(!$this->db->insert('db_item_group',$row)){
                            $flag='false';
                        }
                    ...
                    if(!$flag){
                        ...
                        $this->db->trans_commit();
                        echo "success";
                        $this->session->set_flashdata('success', 'Success!! Item Modifiers Group  Imported Successfully!');
                    }
                    fclose($file);
                }
	   ...
	   $this->permission_check('import_items');
		$data=$this->data;
		$data['page_title']='Import Modifiers';
		$this->load->view('import/import_modifiers', $data);
	}
	
	public function import_modifiers_csv() {
                extract($this->xss_html_filter(array_merge($this->data)));
                $filename = $_FILES["import_file"]["name"];
                ...
                {   
                	...
                	$config['upload_path']          = './uploads/csv/modifiers';
	                $config['allowed_types']        = 'csv';
	                $this->load->library('upload', $config);
                    ...
                    $file = fopen('uploads/csv/modifiers/'.$file_name,"r");
                    ...
                    while(($importdata = fgetcsv($file, NULL, ",")) !== FALSE){
                        ...
                        $modifier_name=$importdata[0];
                        $modifier_price=$importdata[1];
                        //Validate This modifier already exist or not
                        // $query=$this->db->query("select * from db_modifier where upper(modifier_name)=upper('$modifier_name')");
                        // if($query->num_rows()>0){
                        //     echo "Import Failed!<br>'".$modifier_name."'' Modifier already Exist.<br>Row Number:".$i++;
                        //     exit();
                        // }
                        ...
                        //If any record failed to save flag will be set false,then all records rolled back
                        if(!$this->db->insert('db_modifier',$row)){
                            $flag='false';
                        }
                    ...
                    if(!$flag){
                        ...
                        $this->db->trans_commit();
                        echo "success";
                        $this->session->set_flashdata('success', 'Success!! Modifiers Data Imported Successfully!');
                    }
                    fclose($file);
                }
                    ...
                    while(($importdata = fgetcsv($file, NULL, ",")) !== FALSE){
                      ...
                      //  $item_count=$this->db->query("select count(*) as item_count from db_items where upper(item_name)=upper('$item_name')")->row()->item_count;
                        ...
                        $row = array(
                            ...
                            'system_name'               => $SYSTEM_NAME,
                            'status'                    => 1,
                            'item_image'                => 'uploads/items/'.$importdata[2].'png',
                            'description'               =>!empty($importdata[16])?$importdata[16]:0
                        );
