...
class Product extends MY_Controller {
	...
	public function index()
	{
	    ...
	    $category =self::category_menu();
	     $slag=$this->uri->segment(2);
         $wherearray=array('cat_url_slag'=>$slag);
         $catid=$this->items->getSingleColumnNameMultiple('id','db_category',$wherearray);
	    $data['category_menu'] =$category;
	    $data['catid']=$catid;
		$this->load->view('users/product',$data);
	}
    ...
    public function item_list(){
     ...
     public function item_list_filter(){
         $catid=$_POST['catid'];
         //$price_id=array_values($_POST['price_id']);
         
        $array_values = array_values($_POST['price_id']);
        ...
        $item=$this->items->item_home_filter($catid,$sendMaxPrice,$sendMinPrice);
        foreach ($item as $key => $field) {
            $item[$key]->image =cygen_product_img($field->image_code);
        }
        $data['item']=$item;
