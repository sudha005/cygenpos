...
class Customers_model extends CI_Model {
	...
	var $column_order = array('a.id','a.customer_name','a.mobile','a.email','a.status','a.sales_due','a.sales_return_due','a.offer_id'); //set column field database for datatable orderable
	var $column_search = array('a.id','a.customer_name','a.mobile','a.email','a.status','a.sales_due','a.sales_return_due','a.offer_id'); //set column field database for datatable searchable 
	...
	public function verify_and_save(){
		...
		$query1="insert into db_customers(customer_code,customer_name,mobile,phone,email,
											...
											created_date,created_time,created_by,status,gstin,tax_number,offer_id)
											values('$customer_code','$customer_name','$mobile','$phone','$email',
											'$country',$state,'$city','$postcode','$address','$opening_balance',
											'$SYSTEM_IP','$SYSTEM_NAME',
											'$CUR_DATE','$CUR_TIME','$CUR_USERNAME',1,'$gstin','$tax_number','$offer_id')";
	...
	public function update_customers(){
		...
		if($query->num_rows()>0){
			...
			$query1="update db_customers set customer_name='$customer_name',mobile='$mobile',phone='$phone',
							...
							postcode='$postcode',address='$address',gstin='$gstin',tax_number='$tax_number',offer_id='$offer_id'
							 where id=$q_id";
			if ($this->db->simple_query($query1)){
