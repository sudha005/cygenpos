...
class Sales_model extends CI_Model {
	...
	function update_sales_payment_status($sales_id,$customer_id){
		...
		public function update_orderstatus($id,$status){
		//Filtering XSS and html escape from user inputs 
		extract($this->security->xss_clean(html_escape(array_merge($this->data,$_POST))));

			$query1="update db_sales set order_status='$order_status' where id=$id";
	...
	public function view_payments_modal($sales_id){
	 ...
	 // online
		function get_datatables_online($ordertype)
	{
		$this->_get_datatables_query();
		$this->db->where('a.order_type',$ordertype);
		if($_POST['length'] != -1)
		$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
