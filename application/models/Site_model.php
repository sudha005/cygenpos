...
class Site_model extends CI_Model {
    public function get_details(){
		...
		if($query->num_rows()==0){
            ...
            $data['show_upi_code']=$query->show_upi_code;
             $data['uber']=$query->uber;
            $data['menulog']=$query->menulog;
            $data['door_dash']=$query->door_dash;
            $data['open_time']=$query->open_time;
            $data['close_time']=$query->close_time;
            /* QUERY 2*/
			$query1=$query1->row();
	...
	public function update_site(){
        ...
        sales_discount='$sales_discount',change_return=$change_return,show_upi_code=$show_upi_code, round_off=$round_off,sales_invoice_format_id=$sales_invoice_format_id ,sales_invoice_footer_text='$sales_invoice_footer_text'
         $logo,uber=$uber,menulog=$menulog,door_dash=$door_dash,close_time='$close_time',open_time='$open_time',catbgimg_pos='$catbgimg_pos' where id=$q_id";
