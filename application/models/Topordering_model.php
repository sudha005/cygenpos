...
class Topordering_model extends CI_Model {

	var $table = 'contact_walking_form';
		var $column_order = array(null, 'cust_name','cust_mobile','cust_email','cust_ip','cust_system','cust_address','cust_date_added','status','cust_table'); //set column field database for datatable orderable
	var $column_search = array('cust_name','cust_mobile','cust_email','cust_ip','cust_system','cust_address','cust_date_added','status','cust_table'); //set column field database for datatable searchable 
	...
	public function get_details($id,$data){
		//Validate This category already exist or not
		$query=$this->db->query("select * from contact_walking_form where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			$query=$query->row();
			$data['q_id']=$query->id;
			$data['cust_name']=$query->cust_name;
			$data['cust_mobile']=$query->cust_mobile;
			$data['cust_email']=$query->cust_email;
			$data['cust_ip']=$query->cust_ip;
			$data['cust_system']=$query->cust_system;
			$data['cust_address']=$query->cust_address;
			$data['cust_date_added']=$query->cust_date_added;
			$data['status']=$query->status;
			$data['cust_table']=$query->cust_table;
			return $data;
		}
	}
