...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#customerspecificprice-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'customerspecificprice/new_customer_specificprice',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"customerspecificprice/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'customerspecificprice/update_customer_specificprice',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_customer_specificprice(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_customer_specificprice",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
