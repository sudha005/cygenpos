...
$('#save,#update').click(function (e) {
	...
	check_field("offer_name");
	check_field("offer_percentage");	
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#offer-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"offer/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'offer/update_offer',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_offers(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_offers",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
