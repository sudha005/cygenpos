$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#orderstatus-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'sales/updateorderstatus/',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"sales/online_sale";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						e.preventDefault();
						data = new FormData($('#cms-form')[0]);//form name
						/*Check XSS Code*/
						if(!xss_validation(data)){ return false; }
              ...
              //alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"cms/view";
							}
							else if(result=="failed")
							{
								toastr["error"]("Sorry! Failed to save Record.Try again!");
...
//Delete Record start
function delete_cms(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_cms",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
