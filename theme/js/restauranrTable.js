...
$('#save,#update').click(function (e) {
    ...
    check_field("table_name");
    // check_field("table_details");
    check_field("table_capacity");
   ...
   // alert(this_id);
    ...
    {                  
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#resturant-table-form')[0]);//form name
                        /*Check XSS Code*/
                       //alert(data.open_time);
						...
						$.ajax({
						type: 'POST',
						url: base_url+'RestaurantTables/newtable',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             //alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"RestaurantTables/view";
								return;
							}
				...
				//alert('update');
					...
					if(confirm("Do You Wants to Update Record ?")){
                       ...
                       // alert(data);
						...
						$.ajax({
						type: 'POST',
						url: base_url+'RestaurantTables/update_RestaurantTbl',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_restaurentTbl(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_restaurentTbl",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'RestaurantTables/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
